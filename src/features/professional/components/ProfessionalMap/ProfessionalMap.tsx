...
// ── Circular marker factory (with photo or initial) ────────────────────────────
...
// ── MapController: markers + position tracking (must be inside MapContainer) ───
...
function MapController({ professionals, selected, onSelect, onPositionChange }: MapControllerProps) {
  const map = useMap()
  ...
  // Track screen coordinates of selected marker for floating card
  useEffect(() => {
    if (!selected) {
...
// ── FloatingCard: React card that floats above the selected marker ─────────────
...
function FloatingCard({ professional: p, pos, containerSize, onClose, onAddToTeam }: FloatingCardProps) {
  const color = PROFESSION_COLORS[p.profession] ?? '#E07B2A'
  const MARKER_TIP = 10  // px offset above marker tip

  // Clamp horizontally so card doesn't overflow
  const rawLeft = pos.x - CARD_WIDTH / 2
  const left = Math.max(8, Math.min(rawLeft, containerSize.w - CARD_WIDTH - 8))

  // If too close to top, show card BELOW marker; otherwise above
  ...
  const arrowUp = showBelow  // arrow points UP when card is below
  ...
  return (
    ...
    >
      {/* Arrow up (when card is below marker) */}
      {arrowUp && (
      ...
      <div style={{
        ...
        {/* Colored top accent */}
        <div style={{ height: 3, background: `linear-gradient(to right, ${color}cc, ${color}33)` }} />
        ...
        <div style={{ padding: '14px 16px 16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
            ...
            <div style={{
              ...
              background: `${color}22`, color, fontWeight: 800, fontSize: 15,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <p style={{ color: '#F5F0EB', fontWeight: 700, fontSize: 14, margin: 0, lineHeight: 1.3 }}>
                  {p.name}
                </p>
                <button
                  onClick={onClose}
                  style={{
                    flexShrink: 0, background: 'rgba(255,255,255,0.07)', border: 'none',
                    color: 'rgba(245,240,235,0.45)', cursor: 'pointer',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                <span style={{
                  ...
                  background: `${color}20`, color, letterSpacing: '0.02em',
                }}>{p.profession}</span>
                {p.available
                  ? <span style={{ color: '#4CAF50', fontSize: 10, display: 'flex', alignItems: 'center', gap: 3 }}>
          ...
          {/* Stats row */}
          <div style={{
            display: 'flex', gap: 10, marginBottom: 9, fontSize: 11,
            color: 'rgba(245,240,235,0.5)', flexWrap: 'wrap',
          }}>
            <span>
              <span style={{ color: '#FFD166' }}>★ {p.rating}</span>
              <span style={{ color: 'rgba(245,240,235,0.35)' }}> ({p.reviewCount})</span>
            </span>
            <span>📍 {p.distanceKm} km · {p.location.neighborhood}</span>
            <span>{p.completedJobs} obras</span>
          </div>

          {/* Bio */}
          <p style={{
            color: 'rgba(245,240,235,0.48)', fontSize: 11, lineHeight: 1.55,
            margin: '0 0 12px', maxHeight: 58, overflowY: 'auto',
            paddingRight: 2,
          }}>
            {p.bio}
          </p>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <a
              ...
              style={{
                flex: 1, padding: '8px 0', borderRadius: 10, background: '#25D366',
            ...
            {onAddToTeam && (
              <button
                ...
                style={{
                  flex: 1, padding: '8px 0', borderRadius: 10,
                  background: 'rgba(255,255,255,0.06)', color: 'rgba(245,240,235,0.8)',
                  fontWeight: 700, fontSize: 11, border: `1px solid ${color}30`, cursor: 'pointer',
                }}
              >
                + Equipe
              </button>
      ...
      {/* Arrow down (when card is above marker) */}
      {!arrowUp && (
        <div style={{ display: 'flex', justifyContent: `${left === rawLeft ? 'center' : `${pos.x - left}px`}` }}>
          <div style={{
            width: 0, height: 0,
            borderLeft: '9px solid transparent',
            borderRight: '9px solid transparent',
...
export function ProfessionalMap({ professionals, selected, onSelect, onDeselect, onAddToTeam, flyTo }: ProfessionalMapProps) {
  ...
  return (
    <>
      ...
      <div ref={wrapperRef} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
        ...
        {/* Floating card — rendered outside Leaflet but inside wrapper for correct coords */}
        {selected && markerPos && (
          <FloatingCard
