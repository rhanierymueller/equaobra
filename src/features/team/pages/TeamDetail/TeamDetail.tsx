...
function MemberRow({ member: m, memberCost, isOwner, onSetLeader, onRemove, onEditProfession, onChat }: MemberRowProps) {
  ...
  const BTN = { width: 34, height: 34, borderRadius: 10, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as const
      ...
      <div className="flex items-center gap-2.5 px-3 pt-2.5 pb-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            {m.isLeader && <span className="text-xs px-1.5 py-0.5 rounded font-bold shrink-0" style={{ background: 'rgba(224,123,42,0.18)', color: '#E07B2A' }}>líder</span>}
            {isOwner && <span className="text-xs px-1.5 py-0.5 rounded font-medium shrink-0" style={{ background: 'rgba(116,185,255,0.1)', color: '#74B9FF', border: '1px solid rgba(116,185,255,0.2)' }}>você</span>}
          </div>
        </div>
      </div>

      {/* Bottom row: profession (editable) + rate/cost + action buttons */}
      <div className="flex items-center gap-2 px-3 pb-2.5">
        <div className="flex-1 min-w-0">
          {editing ? (
            <div className="flex items-center gap-1">
              <input autoFocus value={profValue}
                onChange={e => setProfValue(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') submitEdit(); if (e.key === 'Escape') setEditing(false) }}
                className="text-xs px-2 py-1 rounded-lg outline-none"
                style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(224,123,42,0.4)', color: '#F5F0EB', width: 120 }}
              />
              <button onClick={submitEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4CAF50', fontSize: 13 }}>✓</button>
              <button onClick={() => setEditing(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(245,240,235,0.4)', fontSize: 13 }}>✕</button>
            </div>
          ) : (
            <div className="flex items-center gap-1 text-xs overflow-hidden" style={{ color: 'rgba(245,240,235,0.4)' }}>
              <span className="truncate" style={{ color }}>{m.profession}</span>
              <button onClick={() => { setProfValue(m.profession); setEditing(true) }} title="Editar função"
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(245,240,235,0.2)', padding: 0, flexShrink: 0 }}>
              ...
              {memberCost && <span className="shrink-0">· R$ {memberCost.toLocaleString('pt-BR')}</span>}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {!isOwner && m.phone !== '' && (
            <a href={`https://wa.me/55${m.phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
              style={{ ...BTN, background: 'rgba(37,211,102,0.1)', color: '#25D366', textDecoration: 'none' }} title="WhatsApp">
          ...
          {!isOwner && (
            <button onClick={onChat} title="Mensagem" style={{ ...BTN, background: 'rgba(116,185,255,0.1)', color: '#74B9FF' }}>
          ...
          {!m.isLeader && (
            <button onClick={onSetLeader} title="Definir como líder" style={{ ...BTN, background: 'rgba(224,123,42,0.1)', color: '#E07B2A' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
              </svg>
            </button>
          )}
          <button onClick={onRemove} title="Remover" style={{ ...BTN, background: 'rgba(229,57,53,0.08)', color: '#FF6B6B' }}>
            <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2 2L10 10M10 2L2 10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg>
          </button>
        </div>
      </div>
    </div>
...
export function TeamDetail({ id }: { id: string }) {
  ...
  function confirm(cfg: DialogConfig) { setDialog(cfg) }
  ...
  if (!team) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-3" style={{ background: '#0D0C0B' }}>
        <p className="text-white font-semibold">Equipe não encontrada</p>
        <button onClick={() => router.push('/profile')} style={{ color: '#E07B2A', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14 }}>← Voltar</button>
      </div>
    )
  }
  ...
  function handleDelete() {
    confirm({
      ...
      variant: 'danger',
      onConfirm: () => { deleteTeam(team!.id); router.push('/profile') },
    })
  }
  ...
  return (
    <div style={{ background: '#0D0C0B', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <button onClick={handleDelete} className="text-xs" style={{ color: '#FF6B6B', background: 'none', border: 'none', cursor: 'pointer' }}>
          Excluir equipe
        </button>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div className="py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <h1 className="font-bold text-white text-2xl mb-1" style={{ letterSpacing: '-0.02em' }}>{team.name}</h1>
          <p className="text-sm mb-5" style={{ color: 'rgba(245,240,235,0.4)' }}>📍 {team.obraLocation}</p>
          ...
          <div className="grid grid-cols-3 gap-3" style={{ maxWidth: 480 }}>
            <div className="rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-1" style={{ color: 'rgba(245,240,235,0.35)' }}>Início</p>
              <p className="text-sm font-bold text-white">{formatDate(team.scheduledStart)}</p>
            </div>
            <div className="rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-1" style={{ color: 'rgba(245,240,235,0.35)' }}>Duração</p>
              <p className="text-sm font-bold text-white">{team.estimatedDays} <span className="font-normal text-xs" style={{ color: 'rgba(245,240,235,0.5)' }}>dias</span></p>
            </div>
            <div className="rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-1" style={{ color: 'rgba(245,240,235,0.35)' }}>Custo total</p>
              <p className="text-sm font-bold" style={{ color: total != null ? '#FFD166' : 'rgba(245,240,235,0.5)' }}>
          ...
          {team.observations && (
            <p className="text-sm mt-4" style={{ color: 'rgba(245,240,235,0.45)' }}>{team.observations}</p>
          )}
        </div>
        ...
        <div className="py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-white text-base">
              Membros <span className="text-sm font-normal" style={{ color: 'rgba(245,240,235,0.3)' }}>({team.members.length})</span>
            </h2>
            {leader && (
              <div className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(245,240,235,0.35)' }}>
            ...
            {team.members.map((member, i) => (
              <MemberRow
                ...
                onSetLeader={() => confirm({
                  ...
                  description: 'O líder atual perderá o cargo. O novo líder terá permissão para gerenciar todos os registros de horas.',
                ...
                onRemove={() => confirm({
                  ...
                  description: 'O membro será removido da equipe. Esta ação não pode ser desfeita.',
                  confirmLabel: 'Remover',
                  variant: 'danger',
                  onConfirm: () => removeMember(team.id, member.professionalId),
                })}
                onEditProfession={profession => updateMemberProfession(team.id, member.professionalId, profession)}
