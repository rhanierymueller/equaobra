...
const DATE_RANGE_LABELS: Record<DateRange, string> = {
  '7d': '7d', '14d': '14d', '30d': '30d', 'month': 'Mês',
}
...
function shortDay(iso: string) { return iso.split('-')[2] }
...
function weekday(iso: string) { return WEEK_LABELS[new Date(iso + 'T12:00:00').getDay()] }

// Show label every N days depending on range length
...
interface ChartSeries { memberId: string; color: string; data: Record<string, number> }
...
function BarChart({ days, series }: { days: string[]; series: ChartSeries[] }) {
  ...
  if (!hasAnyData) {
    return (
      <div style={{ height: H + LABEL_H + 4, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(245,240,235,0.12)" strokeWidth="1.5" strokeLinecap="round">
    ...
    <div>
      <div style={{ overflowX: 'auto', overflowY: 'hidden' }}>
        <svg width={svgW} height={H + LABEL_H} style={{ display: 'block', minWidth: svgW }}>
          {/* Subtle grid */}
          {[0.5, 1].map(f => (
            <line key={f} x1={SIDE_PAD} y1={H - H * f} x2={svgW - SIDE_PAD} y2={H - H * f}
          ...
          {days.map((day, di) => {
            ...
            return (
              ...
              >
                {isHover && (
                  <rect x={gx - 3} y={2} width={GROUP_W - GROUP_GAP + 6} height={H - 2}
                ...
                {series.map((s, si) => {
                  ...
                  return (
                    <rect key={s.memberId}
                      x={x} y={H - bH} width={BAR_W} height={bH} rx={3}
                ...
                {(showLabel || isHover) && (
                  <text
                    ...
                    y={H + 13} textAnchor="middle" fontSize={9}
        ...
        {hoverDay && (() => {
          ...
          return (
            <span style={{ fontSize: 11 }}>
              <span style={{ color: 'rgba(245,240,235,0.35)' }}>{formatDateFull(hoverDay)} — </span>
...
function StatCard({ label, value, color, progress }: { label: string; value: string; color?: string; progress?: number }) {
  return (
    <div className="rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <p className="text-sm font-bold" style={{ color: color ?? 'rgba(245,240,235,0.3)' }}>{value}</p>
      {progress !== undefined && (
...
export function WorkLogSection({ team, currentUser, isLeader, leaderId, onRequestConfirm }: WorkLogSectionProps) {
  ...
  const displayName = team.members.find(m => m.professionalId === displayId)?.name ?? currentUser.name
  ...
  function handleAdd() {
    ...
    if (!h || h <= 0 || h > 24) { setFormError('Horas inválidas (0.5–24)'); return }
    setFormError('')
    const targetId = isLeader && view === 'team' ? selectedMemberId : currentUser.id
    const targetName = team.members.find(m => m.professionalId === targetId)?.name ?? currentUser.name
    const doAdd = () => {
      addLog(targetId, targetName, form.date, h, form.description || undefined)
      if (isLeader && targetId !== currentUser.id) {
        push('log_edited', targetId, team.id, team.name, currentUser.id, currentUser.name,
  ...
  function handleLeaderDelete(logId: string, logOwnerId: string, logDate: string, logHours: number) {
    onRequestConfirm({
      ...
      onConfirm: () => {
        ...
        if (logOwnerId !== currentUser.id) {
          push('log_deleted', logOwnerId, team.id, team.name, currentUser.id, currentUser.name,
  ...
  function handleRequestDelete(logId: string, logDate: string, logHours: number) {
    ...
    push('log_delete_request', leaderId, team.id, team.name, currentUser.id, currentUser.name,
  ...
  const chartSeries: ChartSeries[] = (() => {
    if (!isLeader || view === 'my') {
      ...
      const color = PROFESSION_COLORS[team.members.find(m => m.professionalId === currentUser.id)?.profession as keyof typeof PROFESSION_COLORS] ?? '#E07B2A'
      return [{ memberId: currentUser.id, color, data }]
    }
    return team.members.map(m => {
      const data: Record<string, number> = {}
      for (const l of getLogsForMember(m.professionalId)) data[l.date] = (data[l.date] ?? 0) + l.hours
  ...
  return (
    <div className="rounded-2xl" style={{ border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(13,12,11,0.6)', overflow: 'hidden' }}>
      ...
      >
        <div className="flex items-center gap-2">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#E07B2A" strokeWidth="2.2" strokeLinecap="round">
            <circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" />
          </svg>
          <span className="text-sm font-bold text-white">Registro de Horas</span>
          <span style={{ fontSize: 10, padding: '1px 7px', borderRadius: 99, background: 'rgba(224,123,42,0.1)', color: 'rgba(224,123,42,0.7)', border: '1px solid rgba(224,123,42,0.2)' }}>
            opcional
          </span>
        </div>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
          style={{ transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', color: 'rgba(245,240,235,0.25)' }}>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '12px 16px 16px' }}>
          ...
          {/* Tabs — leader only */}
          {isLeader && (
            <div className="flex mb-4" style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 3 }}>
          ...
          {isLeader && view === 'team' && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {team.members.map(m => {
                ...
                return (
                  <button key={m.professionalId} onClick={() => setSelectedMemberId(m.professionalId)}
                    className="flex items-center gap-1.5 text-xs font-medium"
                    style={{
                      padding: '4px 10px', borderRadius: 99,
                      background: sel ? `${color}20` : 'rgba(255,255,255,0.04)',
                      border: `1px solid ${sel ? color + '50' : 'rgba(255,255,255,0.07)'}`,
                      color: sel ? color : 'rgba(245,240,235,0.45)', cursor: 'pointer',
                    }}
                  >
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, flexShrink: 0, opacity: sel ? 1 : 0.4 }} />
                    {m.name.split(' ')[0]}
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            <StatCard label="Últimos 7 dias" value={`${weekHours}h`} color={weekHours > 0 ? '#E07B2A' : undefined} />
            <StatCard label="Total registrado" value={`${totalHours}h`} color={totalHours > 0 ? '#FFD166' : undefined} progress={progressPct} />
            <StatCard label="Meta da obra" value={`${goalHours}h`} color="#4CAF50" />
          </div>

          {/* Chart */}
          <div className="mb-3" style={{ background: 'rgba(0,0,0,0.25)', borderRadius: 12, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.04)' }}>
            <div className="flex items-center justify-between mb-2.5">
              <span style={{ fontSize: 10, color: 'rgba(245,240,235,0.3)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Horas por dia</span>
                ...
                {(Object.keys(DATE_RANGE_LABELS) as DateRange[]).map(r => (
                  <button key={r} onClick={() => setDateRange(r)}
                    style={{
                      ...
                      cursor: 'pointer', fontWeight: dateRange === r ? 700 : 400,
                    }}
                  >
                    {DATE_RANGE_LABELS[r]}
                  </button>
            ...
            {/* Legend */}
            {isLeader && view === 'team' && (
              <div className="flex flex-wrap gap-3 mt-2 pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                {team.members.map(m => {
                  const color = PROFESSION_COLORS[m.profession as keyof typeof PROFESSION_COLORS] ?? '#E07B2A'
                  return (
                    <div key={m.professionalId} className="flex items-center gap-1.5" style={{ fontSize: 10, color: 'rgba(245,240,235,0.4)' }}>
          ...
          {/* Compact add form */}
          <div className="mb-3" style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.06)' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(245,240,235,0.45)', marginBottom: 8 }}>
              {isLeader && view === 'team'
            ...
            <div className="flex gap-2 items-end">
              <div className="flex flex-col gap-1" style={{ width: 130 }}>
                ...
                <input type="date" value={form.date} max={today()}
                  ...
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: '#F5F0EB', borderRadius: 8, padding: '5px 8px', fontSize: 11, outline: 'none', width: '100%' }}
                />
              </div>
              <div className="flex flex-col gap-1" style={{ width: 60 }}>
                <label style={{ fontSize: 10, color: 'rgba(245,240,235,0.35)' }}>Horas</label>
                <input type="number" min="0.5" max="24" step="0.5" value={form.hours} placeholder="8"
                  onChange={e => setForm(f => ({ ...f, hours: e.target.value }))}
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: '#F5F0EB', borderRadius: 8, padding: '5px 8px', fontSize: 11, outline: 'none', width: '100%' }}
                />
              </div>
              <input type="text" value={form.description} placeholder="Descrição (opcional)"
                onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: '#F5F0EB', borderRadius: 8, padding: '5px 10px', fontSize: 11, outline: 'none' }}
              />
              <button onClick={handleAdd}
                style={{ background: '#E07B2A', color: 'white', border: 'none', borderRadius: 8, padding: '5px 12px', fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}
              >
                + Registrar
              </button>
            </div>
            {formError && <p style={{ fontSize: 11, color: '#FF6B6B', marginTop: 6 }}>{formError}</p>}
          </div>

          {/* Logs list */}
          {displayLogs.length > 0 ? (
            <div>
              <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(245,240,235,0.35)', marginBottom: 6 }}>
                {isLeader && view === 'team' ? `Registros — ${displayName.split(' ')[0]}` : 'Meus registros'}
              </p>
              <div className="flex flex-col gap-1">
                {[...displayLogs].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 8).map(log => {
                  const requested = requestedLogs.has(log.id)
                  return (
                    <div key={log.id} className="flex items-center gap-3"
                      style={{ padding: '6px 10px', borderRadius: 10, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)' }}
                    >
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#E07B2A', minWidth: 28 }}>{log.hours}h</span>
                      <span style={{ fontSize: 10, color: 'rgba(245,240,235,0.3)', minWidth: 42 }}>{formatDate(log.date)}</span>
                      <span className="flex-1 truncate" style={{ fontSize: 11, color: 'rgba(245,240,235,0.45)' }}>{log.description ?? '—'}</span>
                      {isLeader ? (
                        <button onClick={() => handleLeaderDelete(log.id, log.memberId, log.date, log.hours)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(245,240,235,0.2)', padding: '0 2px', flexShrink: 0, lineHeight: 1 }}>
                          <svg width="9" height="9" viewBox="0 0 12 12" fill="none">
                            <path d="M2 2L10 10M10 2L2 10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                          </svg>
                        </button>
                      ) : (
                        <button onClick={() => !requested && handleRequestDelete(log.id, log.date, log.hours)}
                          disabled={requested}
                          style={{
                            fontSize: 10, padding: '2px 7px', borderRadius: 6, flexShrink: 0,
                            background: requested ? 'rgba(76,175,80,0.1)' : 'rgba(255,255,255,0.05)',
            ...
            <p style={{ fontSize: 11, color: 'rgba(245,240,235,0.25)', textAlign: 'center', padding: '8px 0' }}>
