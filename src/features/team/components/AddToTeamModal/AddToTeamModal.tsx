  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-medium" style={{ color: 'rgba(245,240,235,0.55)' }}>
        {label}{required && <span style={{ color: '#E07B2A' }}> *</span>}
      </label>
      <input
        type={type}
        value={value}
...
export function AddToTeamModal({ professional: p, user, onClose, onSuccess }: AddToTeamModalProps) {
  ...
  // Bloquear acesso para profissionais — todos os hooks acima, retorno condicional depois
  if (user.role !== 'contratante') {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
        <div className="rounded-2xl p-6 text-center max-w-sm w-full" style={{ background: 'rgba(20,18,16,0.98)', border: '1px solid rgba(255,255,255,0.1)' }}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: 'rgba(229,57,53,0.1)', border: '1px solid rgba(229,57,53,0.2)' }}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2" strokeLinecap="round">
          ...
          <p className="text-sm mb-5" style={{ color: 'rgba(245,240,235,0.45)' }}>
            Para montar equipes você precisa de uma conta de contratante. Altere seu tipo de conta no perfil.
          </p>
          <button onClick={onClose}
            className="w-full py-2.5 rounded-xl text-sm font-semibold"
            style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(245,240,235,0.7)', border: '1px solid rgba(255,255,255,0.09)', cursor: 'pointer' }}>
            Fechar
          </button>
        </div>
      </div>
  ...
  function handleCreate() {
    if (!form.name.trim()) { setError('Nome da equipe é obrigatório'); return }
    if (!form.obraLocation.trim()) { setError('Local da obra é obrigatório'); return }
    if (!form.scheduledStart) { setError('Data de início é obrigatória'); return }
    if (!form.estimatedDays || Number(form.estimatedDays) < 1) { setError('Informe o tempo estimado'); return }
    setError('')
    setLoading(true)
    setTimeout(() => {
      const ownerMember = {
        ...
        name: user.name,
        profession: user.profession || (user.role === 'profissional' ? 'Profissional' : 'Contratante'),
        phone: '',
  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex: 2001, width: '100%', maxWidth: 460,
        margin: '0 16px',
      }}>
        <div style={{
          ...
          {/* Accent */}
          <div style={{ height: 3, background: `linear-gradient(to right, ${color}, ${color}44)` }} />
          ...
          <div className="flex items-start justify-between p-5 pb-4">
            <div className="flex items-center gap-3">
              <div style={{
                ...
                background: `${color}18`, color, fontWeight: 800, fontSize: 14,
              }}>
                {p.avatarUrl
            ...
            <button
              ...
              style={{
                ...
                color: 'rgba(245,240,235,0.4)', width: 28, height: 28,
          ...
          <div className="px-5 pb-5">
              ...
              <div className="flex flex-col items-center gap-4 py-6 text-center">
                ...
                <div className="flex gap-2 w-full">
                  ...
                  <button
                    ...
                    style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(245,240,235,0.6)', border: '1px solid rgba(255,255,255,0.08)' }}
                  >
                    Fechar
                  </button>
                </div>
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs mt-0.5" style={{ color: 'rgba(245,240,235,0.35)' }}>
                              {team.obraLocation} · {team.members.length} {team.members.length === 1 ? 'membro' : 'membros'}
                            </p>
                          </div>
                          {alreadyIn ? (
                            <span className="text-xs font-medium px-2.5 py-1 rounded-lg" style={{ color: '#4CAF50', background: 'rgba(76,175,80,0.1)' }}>
                              Adicionado ✓
                            </span>
                          ) : (
                            <button
            ...
            {!successTeam && tab === 'new' && (
              <div className="flex flex-col gap-3">
                <Field
                  label="Nome da equipe" required
                  value={form.name} placeholder="Ex: Reforma Apartamento"
                  onChange={v => setForm(f => ({ ...f, name: v }))}
                />
                <Field
                  label="Local da obra" required
                  value={form.obraLocation} placeholder="Endereço ou bairro"
                  onChange={v => setForm(f => ({ ...f, obraLocation: v }))}
                />
                <div className="grid grid-cols-2 gap-3">
                  <Field
                    label="Data de início" required type="date"
                    value={form.scheduledStart} placeholder=""
                    onChange={v => setForm(f => ({ ...f, scheduledStart: v }))}
                  />
                  <Field
                    label="Duração estimada (dias)" required type="number"
                    value={form.estimatedDays} placeholder="Ex: 15"
                    onChange={v => setForm(f => ({ ...f, estimatedDays: v }))}
                  />
                </div>
                <Field
                  label="Observações"
                  value={form.observations} placeholder="Detalhes da obra, requisitos..."
                ...
                <button
                  ...
                  style={{ background: loading ? `${color}77` : color, border: 'none', cursor: loading ? 'default' : 'pointer' }}
                >
