  ...
  return (
    <div className="rounded-2xl overflow-hidden mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="px-5 py-3 flex items-center gap-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ width: 3, height: 14, borderRadius: 99, background: '#E07B2A', flexShrink: 0 }} />
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-4 last:mb-0">
      <label className="block text-xs font-medium mb-1.5" style={{ color: 'rgba(245,240,235,0.4)' }}>{label}</label>
      {children}
    </div>
  )
}

function InputField({ style, onFocus, onBlur, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
...
export function ProfilePage() {
  ...
  async function handleCepChange(raw: string) {
    ...
    if (digits.length === 8) {
      ...
      setCepLoading(false)
      if (result) { setStreet(result.street); setNeighborhood(result.neighborhood); setCity(result.city); setAddrState(result.state) }
  ...
  async function handleCompanyCepChange(raw: string) {
    ...
    if (digits.length === 8) {
      ...
      if (result) { setCompanyStreet(result.street); setCompanyNeighborhood(result.neighborhood); setCompanyCity(result.city); setCompanyAddrState(result.state) }
  ...
  async function handleSaveProfile() {
    ...
    if (roles.length === 0) { setSaveError('Selecione um tipo de conta'); setShowSaveConfirm(false); return }

    if (isCont) {
      if (!companyName.trim()) { setSaveError('Nome da empresa é obrigatório'); setShowSaveConfirm(false); return }
      if (!cnpj.trim()) { setSaveError('CNPJ é obrigatório'); setShowSaveConfirm(false); return }
      const cnpjDigits = cnpj.replace(/\D/g, '')
      if (cnpjDigits.length !== 14) { setSaveError('CNPJ deve ter 14 dígitos'); setShowSaveConfirm(false); return }
      if (website.trim() && !/^https?:\/\/.+\..+/.test(website.trim())) { setSaveError('URL do site inválida (ex: https://site.com)'); setShowSaveConfirm(false); return }
      if (instagram.trim() && /\s/.test(instagram.trim())) { setSaveError('Instagram inválido'); setShowSaveConfirm(false); return }
  ...
  // ── Opportunity handlers (contratante only)
  ...
  function handleChangePassword() {
    ...
    if (pwCurrent !== stored) { setPwError('Senha atual incorreta'); return }
  ...
  return (
    <div style={{ background: '#0D0C0B', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <span className="text-xs font-medium" style={{ color: 'rgba(245,240,235,0.25)' }}>Meu perfil</span>
        <div style={{ width: 40 }} />
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 20px 40px' }}>
        ...
        {/* ── Avatar + name header ──────────────────────────────── */}
        <div className="py-6 flex items-center gap-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="rounded-full flex items-center justify-center font-bold text-xl shrink-0"
            style={{ width: 64, height: 64, background: 'rgba(224,123,42,0.15)', color: '#E07B2A', border: '2px solid rgba(224,123,42,0.45)', boxShadow: '0 0 0 4px rgba(224,123,42,0.08), 0 4px 20px rgba(224,123,42,0.12)' }}>
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-white text-xl leading-tight" style={{ letterSpacing: '-0.02em' }}>{displayName}</h1>
            <p className="text-sm mt-0.5" style={{ color: 'rgba(245,240,235,0.4)' }}>{user.email}</p>
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              {isPro && (
                <span className="text-xs px-2.5 py-0.5 rounded-full font-medium" style={{ background: 'rgba(224,123,42,0.12)', color: '#E07B2A', border: '1px solid rgba(224,123,42,0.25)' }}>Profissional</span>
              )}
              {isCont && (
                <span className="text-xs px-2.5 py-0.5 rounded-full font-medium" style={{ background: 'rgba(76,175,80,0.12)', color: '#66BB6A', border: '1px solid rgba(76,175,80,0.25)' }}>Contratante</span>
              )}
              {isPro && user.showHourlyRate !== false && user.hourlyRate && (
                <span className="text-xs px-2.5 py-0.5 rounded-full font-medium" style={{ background: 'rgba(255,209,102,0.1)', color: '#FFD166', border: '1px solid rgba(255,209,102,0.2)' }}>
                  R$ {user.hourlyRate}/h
                </span>
              )}
              {user.address?.city && (
                <span className="text-xs px-2.5 py-0.5 rounded-full font-medium" style={{ background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,235,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div className="flex items-center gap-3 mt-2">
              <span className="text-xs" style={{ color: 'rgba(245,240,235,0.28)' }}>
                {user.createdAt ? `Desde ${formatDate(user.createdAt.slice(0, 10))}` : 'Novo membro'}
              </span>
        ...
        <div className="pt-5">
          ...
          <Section title="Dados pessoais">
            <Field label="Nome completo">
              <InputField value={name} onChange={e => setName(e.target.value)} placeholder="Seu nome" />
            </Field>

            <Field label="Tipo de conta">
              <p className="text-xs mb-2" style={{ color: 'rgba(245,240,235,0.3)' }}>Selecione apenas um tipo</p>
              <div className="flex gap-2">
                {([
                  { v: 'profissional' as UserRole, l: 'Profissional', desc: 'Quero trabalhar em obras' },
                  { v: 'contratante' as UserRole, l: 'Contratante', desc: 'Quero montar equipes' },
                ]).map(r => {
                  const sel = roles[0] === r.v
                  return (
                    <button key={r.v} onClick={() => setRoles([r.v])}
                    ...
                    >
                      ...
                      <p className="text-xs mt-0.5" style={{ color: 'rgba(245,240,235,0.35)', fontSize: 10 }}>{r.desc}</p>
                    </button>
                  )
                })}
              </div>
            ...
            {isCont && (
              <>
                <Field label="Nome da empresa *">
                  <InputField value={companyName} onChange={e => setCompanyName(e.target.value)} placeholder="Ex: Construtora Silva Ltda" />
                </Field>
                <Field label="CNPJ *">
                  <InputField value={cnpj} onChange={e => {
                ...
                <Field label="Site (opcional)">
                  <InputField value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://suaempresa.com.br" />
                </Field>
                <Field label="Instagram (opcional)">
                  <div className="flex items-center gap-2">
                    <span className="text-sm" style={{ color: 'rgba(245,240,235,0.4)' }}>@</span>
                    <InputField value={instagram} onChange={e => setInstagram(e.target.value.replace(/^@/, ''))} placeholder="suaempresa" style={{ flex: 1 }} />
                  </div>
                </Field>
                <Field label="Facebook (opcional)">
                  <div className="flex items-center gap-2">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="rgba(245,240,235,0.4)" style={{ flexShrink: 0 }}>
                      <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
                    </svg>
                    <InputField value={facebook} onChange={e => setFacebook(e.target.value)} placeholder="facebook.com/suaempresa ou nome de usuário" style={{ flex: 1 }} />
                  </div>
                </Field>
              </>
            )}
              ...
              <Field label="Profissões">
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {professions.length === 0 && (
                    <p className="text-xs" style={{ color: 'rgba(245,240,235,0.25)' }}>Nenhuma profissão adicionada</p>
                  )}
                  {professions.map(p => (
                    <span key={p} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full"
                      style={{ background: 'rgba(224,123,42,0.12)', color: '#E07B2A', border: '1px solid rgba(224,123,42,0.25)' }}>
                      {p}
                      <button onClick={() => removeProfession(p)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(224,123,42,0.6)', padding: 0, lineHeight: 1, display: 'flex' }}>
                ...
                <div className="flex gap-2">
                  <InputField value={profInput} onChange={e => setProfInput(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addProfession() } }}
                    placeholder="Ex: Pedreiro, Eletricista..."
                    style={{ flex: 1 }} />
                  <button onClick={addProfession}
                    style={{ padding: '9px 14px', borderRadius: 10, background: 'rgba(224,123,42,0.15)', color: '#E07B2A', border: '1px solid rgba(224,123,42,0.3)', cursor: 'pointer', fontSize: 13, fontWeight: 600, flexShrink: 0 }}>
                    + Adicionar
                  </button>
                </div>
              </Field>
            ...
            <Field label="Valor por hora">
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  <span style={{
                    position: 'absolute', top: 3, left: showRate ? 21 : 3,
              ...
              {showRate && (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold" style={{ color: 'rgba(245,240,235,0.5)' }}>R$</span>
                  <InputField type="number" min="0" step="1" value={hourlyRate}
                    onChange={e => setHourlyRate(e.target.value)}
                    placeholder="Ex: 85"
                    style={{ flex: 1 }} />
                  <span className="text-sm" style={{ color: 'rgba(245,240,235,0.4)' }}>/h</span>
                </div>
              )}
            </Field>
            )}
            ...
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: 'rgba(245,240,235,0.4)' }}>Seu endereço</span>
              </div>
              <div className="flex gap-2 mb-2">
                <div style={{ flex: '0 0 150px' }}>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>CEP</label>
                  <div className="relative">
                    <InputField value={cep} onChange={e => handleCepChange(e.target.value)} placeholder="00000-000" maxLength={9} />
                    {cepLoading && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs" style={{ color: '#E07B2A' }}>...</span>}
                  </div>
                  {cepError && <p className="text-xs mt-0.5" style={{ color: '#FF6B6B' }}>{cepError}</p>}
                </div>
                <div style={{ flex: '0 0 80px' }}>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Número</label>
                  <InputField value={addrNumber} onChange={e => setAddrNumber(e.target.value)} placeholder="Nº" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Rua</label>
                  <InputField value={street} onChange={e => setStreet(e.target.value)} placeholder="Rua / Av." disabled={cepLoading} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Bairro</label>
                  <InputField value={neighborhood} onChange={e => setNeighborhood(e.target.value)} placeholder="Bairro" disabled={cepLoading} />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="col-span-2">
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Cidade</label>
                  <InputField value={city} onChange={e => setCity(e.target.value)} placeholder="Cidade" disabled={cepLoading} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Estado</label>
                  <InputField value={addrState} onChange={e => setAddrState(e.target.value)} placeholder="UF" disabled={cepLoading} />
                </div>
              </div>
            </div>
            ...
            {isCont && (
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-3">
                  <div style={{ width: 3, height: 12, borderRadius: 99, background: '#66BB6A', flexShrink: 0 }} />
                  <span className="text-xs font-bold uppercase tracking-wider" style={{ color: 'rgba(245,240,235,0.4)' }}>Endereço da empresa</span>
                </div>
                <div className="flex gap-2 mb-2">
                  <div style={{ flex: '0 0 150px' }}>
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>CEP</label>
                    <div className="relative">
                      <InputField value={companyCep} onChange={e => handleCompanyCepChange(e.target.value)} placeholder="00000-000" maxLength={9} />
                      {companyCepLoading && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs" style={{ color: '#E07B2A' }}>...</span>}
                    </div>
                    {companyCepError && <p className="text-xs mt-0.5" style={{ color: '#FF6B6B' }}>{companyCepError}</p>}
                  </div>
                  <div style={{ flex: '0 0 80px' }}>
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Número</label>
                    <InputField value={companyAddrNumber} onChange={e => setCompanyAddrNumber(e.target.value)} placeholder="Nº" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Rua</label>
                    <InputField value={companyStreet} onChange={e => setCompanyStreet(e.target.value)} placeholder="Rua / Av." disabled={companyCepLoading} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Bairro</label>
                    <InputField value={companyNeighborhood} onChange={e => setCompanyNeighborhood(e.target.value)} placeholder="Bairro" disabled={companyCepLoading} />
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2">
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Cidade</label>
                    <InputField value={companyCity} onChange={e => setCompanyCity(e.target.value)} placeholder="Cidade" disabled={companyCepLoading} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(245,240,235,0.4)' }}>Estado</label>
                    <InputField value={companyAddrState} onChange={e => setCompanyAddrState(e.target.value)} placeholder="UF" disabled={companyCepLoading} />
                  </div>
                </div>
              </div>
            )}

            {saveError && <p className="text-xs mb-3" style={{ color: '#FF6B6B' }}>{saveError}</p>}
            <button onClick={() => setShowSaveConfirm(true)}
              ...
              style={{ background: 'rgba(224,123,42,0.1)', color: '#E07B2A', border: '1px solid rgba(224,123,42,0.25)', cursor: 'pointer' }}>
          ...
          <Section title="Alterar senha">
            <Field label="Senha atual">
              <InputField type="password" value={pwCurrent} onChange={e => setPwCurrent(e.target.value)}
                placeholder="••••••••" />
            </Field>
            <Field label="Nova senha">
              <InputField type="password" value={pwNew} onChange={e => setPwNew(e.target.value)}
                placeholder="Mínimo 6 caracteres" />
            </Field>
            <Field label="Confirmar nova senha">
              <InputField type="password" value={pwConfirm} onChange={e => setPwConfirm(e.target.value)}
                placeholder="••••••••"
                onKeyDown={e => { if (e.key === 'Enter') handleChangePassword() }}
              />
            </Field>
            {pwError && <p className="text-xs mb-3" style={{ color: '#FF6B6B' }}>{pwError}</p>}
            {pwSuccess && <p className="text-xs mb-3" style={{ color: '#4CAF50' }}>Senha alterada com sucesso!</p>}
          ...
          {isCont && (
            <Section title="Estou buscando profissionais">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  <span style={{
                    position: 'absolute', top: 3, left: oppActive ? 23 : 3,
              ...
              {/* Form — sempre visível para preencher antes de ativar */}
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Field label="Localização *">
                    <InputField value={oppLocation} onChange={e => setOppLocation(e.target.value)} placeholder="Ex: São Paulo, SP" />
                  </Field>
                  <Field label="Previsão de início">
                    <InputField type="date" value={oppStart} onChange={e => setOppStart(e.target.value)} placeholder="" />
                  </Field>
                </div>

                <Field label="Duração estimada">
                  <InputField value={oppDuration} onChange={e => setOppDuration(e.target.value)} placeholder="Ex: 30 dias, 2 meses..." />
                </Field>

                <Field label="Profissões necessárias *">
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {oppProfessions.length === 0 && (
                      <p className="text-xs" style={{ color: 'rgba(245,240,235,0.25)' }}>Nenhuma adicionada</p>
                    )}
                    {oppProfessions.map(p => (
                      <span key={p} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full"
                        style={{ background: 'rgba(224,123,42,0.12)', color: '#E07B2A', border: '1px solid rgba(224,123,42,0.25)' }}>
                        {p}
                        <button onClick={() => setOppProfessions(prev => prev.filter(x => x !== p))}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(224,123,42,0.6)', padding: 0, lineHeight: 1, display: 'flex' }}>
                ...
                {oppSaveMsg && (
                  <p className="text-xs px-3 py-2 rounded-xl"
                    style={{ background: oppSaveMsg.startsWith('Publicado') ? 'rgba(76,175,80,0.1)' : 'rgba(229,57,53,0.1)', color: oppSaveMsg.startsWith('Publicado') ? '#4CAF50' : '#FF6B6B', border: `1px solid ${oppSaveMsg.startsWith('Publicado') ? 'rgba(76,175,80,0.2)' : 'rgba(229,57,53,0.2)'}` }}>
                    {oppSaveMsg}
                  </p>
                )}
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(224,123,42,0.1)' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#E07B2A" strokeWidth="1.5" strokeLinecap="round">
            ...
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M5 2l5 5-5 5" stroke="rgba(245,240,235,0.3)" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </Link>
