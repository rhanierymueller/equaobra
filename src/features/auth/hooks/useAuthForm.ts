...
export function useRegisterForm(): UseRegisterFormReturn {
  ...
  const validate = useCallback((v: RegisterCredentials): RegisterErrors => {
    ...
    // Address validation — always required (at least CEP + city)
    ...
    // Company address — only required when contratante
    if (isCont) {
  ...
  const handleChange = useCallback((field: keyof RegisterCredentials, value: string) => {
    setValues(prev => {
      // Special handling: roles is stored as JSON-encoded array from the form
      if (field === 'roles') {
        try {
