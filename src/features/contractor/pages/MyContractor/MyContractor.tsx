...
function InputEl({ style, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props} style={{
      ...
      border: '1px solid rgba(255,255,255,0.09)', color: '#F5F0EB',
      borderRadius: 10, padding: '9px 12px', fontSize: 13, outline: 'none', ...style,
    }} />
  )
}
...
function Stars({ rating }: { rating: number }) {
  return (
    <span className="flex items-center gap-0.5">
      ...
      <span className="text-xs ml-1" style={{ color: 'rgba(245,240,235,0.4)' }}>{rating.toFixed(1)}</span>
    </span>
  )
}
...
const EMPTY_FORM = { description: '', location: '', locationLat: undefined as number | undefined, locationLng: undefined as number | undefined, start: '', duration: '', professions: [] as string[] }
...
function OppForm({ user, editing, onSaved, onCancelEdit, publish, updateOpportunity }: OppFormProps) {
  ...
  // Sync when switching to a different editing target
  useEffect(() => {
    if (editing) {
  ...
  function handleSave() {
    ...
    const data = {
      contractorId: user.id, contractorName: user.name, companyName: user.companyName,
      avatarInitials: initials, obraDescription: description.trim(),
      obraLocation: location.trim(), lat: locationLat, lng: locationLng,
    ...
    <div className="flex flex-col gap-3">
      {isEdit && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl mb-1"
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#74B9FF" strokeWidth="2" strokeLinecap="round">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
          </svg>
          <span className="text-xs font-medium" style={{ color: '#74B9FF' }}>Editando vaga</span>
          <button onClick={onCancelEdit} className="ml-auto text-xs" style={{ color: 'rgba(116,185,255,0.6)', background: 'none', border: 'none', cursor: 'pointer' }}>
            Cancelar
          </button>
        </div>
      )}
      ...
      <div>
        ...
        <textarea value={description} onChange={e => setDescription(e.target.value)}
          ...
          rows={3}
          style={{ width: '100%', resize: 'none', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: '#F5F0EB', borderRadius: 10, padding: '9px 12px', fontSize: 13, outline: 'none' }}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <LocalityAutocomplete
            value={location}
            onChange={val => { setLocation(val); setLocationLat(undefined); setLocationLng(undefined) }}
            onSelect={(name, lat, lng) => { setLocation(name); setLocationLat(lat); setLocationLng(lng) }}
      ...
      <div>
        ...
        <InputEl value={duration} onChange={e => setDuration(e.target.value)} placeholder="Ex: 30 dias, 2 meses..." />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-2 min-h-[28px]">
          {professions.length === 0
            ? <p className="text-xs self-center" style={{ color: 'rgba(245,240,235,0.25)' }}>Nenhuma adicionada</p>
            : professions.map(p => (
              <span key={p} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full"
                style={{ background: 'rgba(224,123,42,0.12)', color: ACCENT, border: '1px solid rgba(224,123,42,0.25)' }}>
                {p}
                <button onClick={() => setProfessions(prev => prev.filter(x => x !== p))}
        ...
        <div className="flex gap-2">
          <select value={profInput} onChange={e => setProfInput(e.target.value)}
            style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: profInput ? '#F5F0EB' : 'rgba(245,240,235,0.35)', borderRadius: 10, padding: '9px 12px', fontSize: 13, outline: 'none' }}>
          ...
          <button
            onClick={() => { if (profInput && !professions.includes(profInput)) { setProfessions(prev => [...prev, profInput]); setProfInput('') } }}
            style={{ padding: '9px 14px', borderRadius: 10, background: 'rgba(224,123,42,0.15)', color: ACCENT, border: '1px solid rgba(224,123,42,0.3)', cursor: 'pointer', fontSize: 13, fontWeight: 600, flexShrink: 0 }}>
            + Add
          </button>
        </div>
      </div>
      ...
      {msg && (
        <p className="text-xs px-3 py-2 rounded-xl"
          style={{ background: 'rgba(229,57,53,0.1)', color: '#FF6B6B', border: '1px solid rgba(229,57,53,0.2)' }}>
          {msg}
        </p>
      )}
      ...
      <div className="flex gap-2">
        {isEdit && (
          <button onClick={onCancelEdit}
            style={{ flex: 1, padding: '10px 0', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,235,0.6)', border: '1px solid rgba(255,255,255,0.09)', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
            Cancelar
          </button>
        )}
        <button onClick={handleSave}
          style={{ flex: 2, padding: '10px 0', borderRadius: 12, background: ACCENT, color: 'white', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>
  ...
  if (vagas.length === 0) {
    return (
      <div className="py-8 text-center rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.07)' }}>
        <p className="text-sm" style={{ color: 'rgba(245,240,235,0.3)' }}>Nenhuma vaga publicada ainda</p>
        <p className="text-xs mt-1" style={{ color: 'rgba(245,240,235,0.18)' }}>Preencha o formulário acima para criar sua primeira vaga</p>
      </div>
    )
  }
    ...
    <div style={{ maxHeight: 320, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}
      ...
      {vagas.map(v => (
        <div key={v.id} className="rounded-xl overflow-hidden"
          style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
          <div style={{ height: 2, background: `linear-gradient(to right, ${v.active ? ACCENT : 'rgba(255,255,255,0.15)'}, transparent)` }} />
          <div className="p-3.5">
            <div className="flex items-start gap-2 mb-2">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate leading-tight">
                  {v.obraDescription.slice(0, 55)}{v.obraDescription.length > 55 ? '…' : ''}
                </p>
            ...
            <div className="flex flex-wrap gap-1 mb-2.5">
              {v.lookingForProfessions.slice(0, 3).map(p => (
                <span key={p} className="text-xs px-1.5 py-0.5 rounded-full"
                  style={{ background: `${ACCENT}12`, color: ACCENT, border: `1px solid ${ACCENT}22`, fontSize: 10 }}>
                  {p}
                </span>
              ))}
              {v.lookingForProfessions.length > 3 && (
                <span className="text-xs rounded-full" style={{ color: 'rgba(245,240,235,0.3)', fontSize: 10 }}>
                  +{v.lookingForProfessions.length - 3}
                </span>
              )}
            </div>
            ...
            <div className="flex gap-1.5">
              <button onClick={() => onEdit(v)}
                style={{ flex: 1, padding: '6px 0', borderRadius: 8, fontSize: 11, fontWeight: 600, background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,235,0.65)', border: '1px solid rgba(255,255,255,0.09)', cursor: 'pointer' }}>
                Editar
              </button>
              <button onClick={() => onDelete(v.id)}
                style={{ flex: 1, padding: '6px 0', borderRadius: 8, fontSize: 11, fontWeight: 600, background: 'rgba(229,57,53,0.07)', color: '#FF6B6B', border: '1px solid rgba(229,57,53,0.15)', cursor: 'pointer' }}>
                Excluir
              </button>
            </div>
          </div>
...
function CandidatesSection({ contractorId, onChat }: CandidatesSectionProps) {
  ...
  const availableProfessions = Array.from(new Set(interests.map(i => i.profession).filter(Boolean))) as string[]
  ...
  if (interests.length === 0) {
    return (
      <div className="py-10 text-center rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.07)' }}>
        <p className="text-sm font-medium" style={{ color: 'rgba(245,240,235,0.3)' }}>Nenhum candidato ainda</p>
        <p className="text-xs mt-1" style={{ color: 'rgba(245,240,235,0.18)', maxWidth: 240, margin: '4px auto 0' }}>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-2">
        <div className="flex gap-2">
          <select value={filterProfession} onChange={e => setFilterProfession(e.target.value)}
            style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: filterProfession ? '#F5F0EB' : 'rgba(245,240,235,0.3)', borderRadius: 10, padding: '8px 12px', fontSize: 12, outline: 'none' }}>
            <option value="" style={{ background: '#1A1916' }}>Todas as profissões</option>
            {availableProfessions.map(p => <option key={p} value={p} style={{ background: '#1A1916' }}>{p}</option>)}
          </select>
          <select value={filterRating} onChange={e => setFilterRating(Number(e.target.value))}
            style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: filterRating > 0 ? '#F5F0EB' : 'rgba(245,240,235,0.3)', borderRadius: 10, padding: '8px 12px', fontSize: 12, outline: 'none' }}>
        ...
        <InputEl value={filterLocation} onChange={e => setFilterLocation(e.target.value)}
          placeholder="Filtrar por localidade..." style={{ padding: '8px 12px', fontSize: 12 }} />
      </div>
      ...
      <div style={{ maxHeight: 360, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }} className="pr-1">
        {filtered.length === 0 ? (
          <p className="text-xs text-center py-6" style={{ color: 'rgba(245,240,235,0.3)' }}>
            Nenhum candidato com esses filtros
          </p>
        ) : filtered.map(candidate => (
          <div key={candidate.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, background: `${ACCENT}22`, color: ACCENT, fontWeight: 800, fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1.5px solid ${ACCENT}44` }}>
              {candidate.professionalInitials}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">{candidate.professionalName}</p>
              <div className="flex items-center gap-2 flex-wrap mt-0.5">
                {candidate.profession && <span className="text-xs" style={{ color: 'rgba(245,240,235,0.45)' }}>{candidate.profession}</span>}
                {candidate.location && (
                  <span className="text-xs flex items-center gap-0.5" style={{ color: 'rgba(245,240,235,0.3)' }}>
                    <svg width="8" height="8" viewBox="0 0 13 13" fill="none"><path d="M6.5 1a4 4 0 0 1 4 4c0 3.5-4 7.5-4 7.5S2.5 8.5 2.5 5a4 4 0 0 1 4-4z" stroke="currentColor" strokeWidth="1.3" /></svg>
                    {candidate.location}
                  </span>
                )}
              </div>
              {candidate.rating != null && <div className="mt-1"><Stars rating={candidate.rating} /></div>}
            </div>
            <div className="flex flex-col gap-1.5 shrink-0">
              <button onClick={() => onChat(candidate.professionalId, candidate.professionalName, candidate.professionalInitials, candidate.profession)}
                style={{ padding: '6px 14px', borderRadius: 10, background: ACCENT, color: 'white', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12 }}>
                Falar
              </button>
              <Link href={`/professional/${candidate.professionalId}`}
                style={{ padding: '5px 14px', borderRadius: 10, textAlign: 'center', background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,235,0.6)', border: '1px solid rgba(255,255,255,0.09)', fontSize: 11, fontWeight: 600, textDecoration: 'none', display: 'block' }}>
                Ver perfil
              </Link>
            </div>
          </div>
...
export function MyContractor() {
  ...
  const { publish, updateOpportunity, deleteOpportunity, getContractorOpportunities } = useOpportunities()
  ...
  if (!user || (user.role !== 'contratante' && !user.roles?.includes('contratante'))) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4" style={{ background: '#0D0C0B' }}>
        ...
        <Link href="/auth" className="px-5 py-2.5 rounded-xl text-sm font-bold text-white" style={{ background: ACCENT }}>Entrar</Link>
      </div>
    )
  }
  ...
  function handleChat(candidateId: string, name: string, avatarInitials: string, profession?: string) {
    setChatTarget({ professionalId: candidateId, name, avatarInitials, profession: profession ?? 'Profissional' })
  }
  ...
  return (
    <div style={{ background: '#0D0C0B', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <span className="text-xs font-medium" style={{ color: 'rgba(245,240,235,0.25)' }}>Minha construtora</span>
        <div style={{ width: 40 }} />
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 20px 40px' }}>
        ...
        <div className="py-6 flex items-center gap-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ width: 60, height: 60, borderRadius: 16, flexShrink: 0, background: `${ACCENT}22`, color: ACCENT, fontWeight: 800, fontSize: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${ACCENT}55` }}>
            {initials}
          </div>
          <div className="min-w-0">
            <h1 className="font-bold text-white text-xl leading-tight truncate" style={{ letterSpacing: '-0.02em' }}>{displayName}</h1>
            {user.companyName && <p className="text-sm mt-0.5" style={{ color: 'rgba(245,240,235,0.4)' }}>{user.name}</p>}
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full" style={{ background: `${ACCENT}1a`, color: ACCENT, border: `1px solid ${ACCENT}33` }}>Contratante</span>
              <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                style={{ background: hasActiveVaga ? 'rgba(76,175,80,0.1)' : 'rgba(255,255,255,0.04)', color: hasActiveVaga ? '#4CAF50' : 'rgba(245,240,235,0.3)', border: `1px solid ${hasActiveVaga ? 'rgba(76,175,80,0.2)' : 'rgba(255,255,255,0.07)'}` }}>
                {myVagas.length > 0 ? `${myVagas.filter(v => v.active).length} vaga${myVagas.filter(v => v.active).length !== 1 ? 's' : ''} ativa${myVagas.filter(v => v.active).length !== 1 ? 's' : ''}` : 'Sem vagas'}
              </span>
            </div>
          </div>
        </div>
