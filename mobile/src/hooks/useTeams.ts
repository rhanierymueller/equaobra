...
export function useTeams() {
  ...
  const createTeam = useCallback(async (input: Omit<Team, 'id' | 'createdAt' | 'userId' | 'members' | 'workLogs'> & { members?: TeamMember[] }) => {
  ...
  const addWorkLog = useCallback(async (teamId: string, memberId: string, log: {
    memberName: string
    date: string
    hours: number
    note?: string
  ...
  const removeWorkLog = useCallback(async (teamId: string, logId: string) => {
    setTeams(prev => prev.map(t =>
      t.id === teamId ? { ...t, workLogs: (t.workLogs ?? []).filter(l => l.id !== logId) } : t
    ))
    try {
