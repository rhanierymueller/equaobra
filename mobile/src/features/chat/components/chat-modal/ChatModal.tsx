...
export function ChatModal({ prof, visible, onClose }: ChatModalProps) {
  ...
  useEffect(() => {
    ...
    api
      .get<
        Array<{ id: string; professionalId: string; contractorId: string; messages: ChatMessage[] }>
      >('/api/chats')
      .then((chats) => {
    ...
    >
      <View style={{ flex: 1, backgroundColor: colors.backgroundCard }}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text weight="bold" size="base" color="primary">
              {prof.name}
            </Text>
          ...
          {messages.length === 0 ? (
            ...
            messages.map((m) => {
              ...
              return (
                <View
                  key={m.id}
                  style={{ alignItems: mine ? 'flex-end' : 'flex-start', marginBottom: 8 }}
                >
                  <View
                    style={{
                      maxWidth: '80%',
                  ...
                  >
                    <Text weight="regular" size="base" color="primary">
                      {m.text}
                    </Text>
                  </View>
                  <Text
          ...
          >
            {sending ? (
              ...
              <Ionicons
                name="send"
                size={18}
                color={text.trim() ? colors.text.primary : colors.text.hint}
              />
            )}
          </TouchableOpacity>
        </View>
