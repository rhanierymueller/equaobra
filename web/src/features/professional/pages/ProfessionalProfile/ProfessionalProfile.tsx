...
export function ProfessionalProfile({ id }: { id: string }) {
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center" style={{ background: '#0D0C0B' }}>
        <div style={{ width: 32, height: 32, border: '3px solid rgba(224,123,42,0.3)', borderTopColor: '#E07B2A', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
    ...
    return (
      <div className="h-screen flex items-center justify-center" style={{ background: '#0D0C0B' }}>
        <div className="text-center">
          ...
          <button onClick={() => router.push('/home')} style={{ color: '#E07B2A', fontSize: 14 }}>← Voltar</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: '#0D0C0B', minHeight: '100vh' }}>
            ...
            <div style={{ height: 3, background: `linear-gradient(to right, ${color}, ${color}44, transparent)` }} />
        ...
        <span className="text-xs font-medium" style={{ color: 'rgba(245,240,235,0.25)' }}>Perfil do profissional</span>
        <div style={{ width: 52 }} />
      </div>
          ...
          <div className="flex items-start gap-4">
              ...
              background: `${color}22`, color, fontWeight: 800, fontSize: 22,
            }}>
              {p.avatarUrl
              ...
              <h1 className="font-bold text-white text-xl leading-tight mb-1.5" style={{ letterSpacing: '-0.02em' }}>
                {p.name}
              </h1>

              <div className="flex items-center gap-2 flex-wrap mb-2">
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full" style={{ background: `${color}1a`, color, border: `1px solid ${color}33` }}>
                  {p.profession}
                </span>
                {p.available
                  ...
                  : <span className="text-xs" style={{ color: 'rgba(245,240,235,0.3)' }}>Ocupado</span>
                }
              </div>

              <div className="flex items-center gap-3 text-xs flex-wrap" style={{ color: 'rgba(245,240,235,0.4)' }}>
          ...
          <p className="text-sm leading-relaxed" style={{ color: 'rgba(245,240,235,0.6)' }}>{p.bio}</p>
          {p.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {p.tags.map(tag => (
                <span
                  ...
                  style={{ background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,235,0.45)', border: '1px solid rgba(255,255,255,0.08)' }}
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm" style={{ color: 'rgba(245,240,235,0.4)' }}>{p.phone}</span>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs" style={{ color: 'rgba(245,240,235,0.25)' }}>{p.reviewCount} no total</span>
          </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-xs ml-2" style={{ color: 'rgba(245,240,235,0.25)' }}>{rev.date}</span>
                    </div>
                  </div>
                  <StarRow rating={rev.rating} />
                </div>
                <p className="text-sm leading-relaxed pl-9" style={{ color: 'rgba(245,240,235,0.5)' }}>
                  {rev.text}
                </p>
              </div>
            ))}
        ...
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 100 }}>
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold" style={{ background: 'rgba(18,17,14,0.96)', border: `1px solid ${color}44`, color, boxShadow: `0 4px 24px rgba(0,0,0,0.5)`, backdropFilter: 'blur(12px)' }}>
