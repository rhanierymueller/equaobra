  ...
  return {
    ...
    timestamp:
      typeof raw.timestamp === 'string'
        ? raw.timestamp
        : new Date(raw.timestamp as string | number).toISOString(),
  }
}
...
export function useChat(userId: string) {
  ...
  useEffect(() => {
    if (!userId) return
    api
      .get<Record<string, unknown>[]>('/api/chats')
  ...
  const getOrCreateConversation = useCallback(
    (
      ...
      api
        .post<Record<string, unknown>>('/api/chats', {
          professionalId,
          professionalName,
          professionalInitials,
          professionalAvatarUrl,
        ...
        .then((data) => {
          ...
          setConversations((prev) =>
            prev.map((c) => (c.id === mapped.id ? { ...mapped, messages: c.messages } : c)),
          )
        })
        .catch(() => {})
  ...
  const sendMessage = useCallback(
    (conversationId: string, senderId: string, senderName: string, text: string): ChatMessage => {
      ...
      api
        .post<Record<string, unknown>>(`/api/chats/${conversationId}/messages`, {
          senderName,
          text,
        })
        .then((data) => {
